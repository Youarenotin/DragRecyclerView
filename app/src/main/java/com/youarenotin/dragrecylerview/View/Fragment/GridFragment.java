...
public class GridFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.frag_gridview, null, false);
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        ViewAdapter girdAdapter = new ViewAdapter(list, R.layout.item_grid);
        RecyclerView recyclerView = (RecyclerView) view.findViewById(R.id.gridview);
        recyclerView.setLayoutManager(new GridLayoutManager(getActivity(), 4));
        recyclerView.setAdapter(girdAdapter);
        //拖动
        ItemTouchHelper itemTouchHelper = new ItemTouchHelper(new ItemTouchCallBack(girdAdapter));
        ...
        recyclerView.addOnItemTouchListener(new OnItemTouchListenerImpl(recyclerView) {
                                                ...
                                                protected void onItemLongClick(RecyclerView.ViewHolder vh) {
                                                    if (vh instanceof ViewAdapter.mViewholder) {
                                                        ViewAdapter.mViewholder vh_New = (ViewAdapter.mViewholder) vh;
                                                        Toast.makeText(getActivity(), "我能吃->" + vh_New.des.getText(), Toast.LENGTH_SHORT).show();
                                                    }
                                                }
                                            }
        );
